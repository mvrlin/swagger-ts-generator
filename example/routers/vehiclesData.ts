...
const DataVehicleListResponseSchemaSchema = z.lazy(() => data.VehicleListResponseSchema);

const DataFilterOptionSchemaSchema = z.lazy(() => data.FilterOptionSchema);

export const vehiclesDataRouter = router({

  4wk12fy2k39z: protectedProcedure
    .input(z.object({}))
    .mutation(async ({ input, ctx }) => {
      try {
        const response = await ctx.api.4wk12fy2k39z();
        return response;
      } catch (error) {
        throw new TRPCError({
          code: 'BAD_REQUEST',
          message: 'Failed to process 4wk12fy2k39z',
        });
      }
    }),
});

...
const model_UserSchemaSchema = z.object({
    ...
    ID: z.any().optional(),
    createdAt: z.string().optional(),
    deletedAt: z.lazy(() => GormDeletedAtSchemaSchema).optional(),
    email: z.string().optional(),
...
export const manufacturersRouter = router({
  ...
  5svhuuq5b2fq: protectedProcedure
    .input(z.object({}))
    .mutation(async ({ input, ctx }) => {
      try {
        const response = await ctx.api.5svhuuq5b2fq();
        return response;
      } catch (error) {
        throw new TRPCError({
          code: 'BAD_REQUEST',
          message: 'Failed to process 5svhuuq5b2fq',
        });
      }
    }),
});

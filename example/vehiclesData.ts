...
const [object Object] = z.lazy(() => data.VehicleListResponseSchema);
...
const [object Object] = z.lazy(() => data.FilterOptionSchema);
...
export const vehiclesDataRouter = router({
    ...
    s79sc0tzb5e3: protectedProcedure
      .input(z.object({}))
      .mutation(async ({ input, ctx }) => {
        try {
          
          const response = await ctx.api.s79sc0tzb5e3(input);
          return response;
        } catch (error) {
          throw new TRPCError({
            code: 'BAD_REQUEST',
            message: 'Failed to process s79sc0tzb5e3',
          });
        }
      }),
});
